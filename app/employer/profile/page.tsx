...
import { Briefcase, Calendar, Building2, AlertTriangle } from "lucide-react"
...
export default function EmployerProfilePage() {
  ...
  useEffect(() => {
    ...
    if (missing) {
      setTimeout(() => { toast.error("Please complete your profile") }, 300)
    }
  }, [])
  ...
  const validateProfile = () => {
    ...
    if (!location.trim()) { toast.error("Missing location"); return false }
    if (!zipCode.trim() || !zipRegex.test(zipCode)) { toast.error("Invalid zip code — must be 5 digits"); scrollToField(zipRef); return false }
    if (!details.trim()) { toast.error("Missing description"); scrollToField(detailsRef); return false }
    if (!hourlyPay.trim() || Number(hourlyPay) <= 0) { toast.error("Missing hourly pay"); return false }
    ...
    <div className="min-h-screen bg-background p-4 sm:p-8">
        ...
        <div className="mx-auto max-w-6xl">
          <div className="flex items-center justify-between rounded-3xl border border-blue-200 bg-gradient-to-r from-blue-100 via-white to-blue-50 shadow-xl backdrop-blur px-6 py-5 sm:px-7">
            <div className="flex flex-col">
              ...
              <p className="text-sm text-gray-600 mt-1 max-w-md">Add your company details and hiring preferences to start matching with students.</p>
            </div>
            <Button
      ...
      <Card className="mb-4">
        ...
        <CardContent className="space-y-2">
          ...
          <input value={location} onChange={(e) => setLocation(e.target.value)} className="w-full border rounded px-2 py-1 text-sm" placeholder="Street Address" />
          <input
...
<p className="text-xs text-muted-foreground mt-1">
  {zipMatchPrecision === 5
    ...
    : "Show students in your broader region (first 3 digits match)."}
          ...
          />
          <textarea ref={detailsRef} value={details} onChange={(e) => setDetails(e.target.value)} className="w-full border rounded px-2 py-1 text-sm" placeholder="Company Description" />
          <input value={hourlyPay} onChange={(e) => setHourlyPay(e.target.value)} className="w-full border rounded px-2 py-1 text-sm" placeholder="Pay per hour (e.g. 15.50)" type="number" step="0.01" />
          <div className="flex items-center justify-between mt-2">
            <span className="text-sm">Tips?</span>
            <button type="button" onClick={() => setHasTips(prev => !prev)} className="px-4 py-1 text-xs rounded border font-semibold transition-all min-w-[80px] text-center bg-white text-blue-700 border-blue-400">
              {hasTips ? "Yes" : "No"}
            </button>
          </div>
        </CardContent>
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-2 mt-2">
              {["morning", "night", "flexible"].map((type) => (
                <button key={type} onClick={() => setShiftPreference(type as any)}
                  className={`px-3 py-1 text-xs rounded-full border transition-all ${shiftPreference === type ? "bg-blue-100 text-blue-700 border-blue-200 shadow-sm" : "bg-gray-100 text-gray-600 border-gray-200"}`}>
                  {type}
                </button>
              ))}
            </div>
          ...
          <div className="space-y-2">
            {availableShifts.map((day, index) => (
              <div key={day.day} className="flex justify-between items-center text-sm">
                <span className="w-16 font-medium text-gray-700">{day.day.slice(0, 3)}</span>
                <div className="flex gap-1 items-center">
                  <select value={day.start} disabled={!day.active}
                    onChange={(e) => setAvailableShifts((prev) => prev.map((d, i) => i === index ? { ...d, start: e.target.value } : d))}
                    className={`w-24 border rounded text-xs px-1 py-1 text-black bg-white ${!day.active ? "opacity-40 cursor-not-allowed" : ""}`}>
                    {timeOptions.map((t) => <option key={t} value={t}>{t}</option>)}
                  </select>
                  <select value={day.end} disabled={!day.active}
                    onChange={(e) => setAvailableShifts((prev) => prev.map((d, i) => i === index ? { ...d, end: e.target.value } : d))}
                    className={`w-24 border rounded text-xs px-1 py-1 text-black bg-white ${!day.active ? "opacity-40 cursor-not-allowed" : ""}`}>
                    {timeOptions.map((t) => <option key={t} value={t}>{t}</option>)}
                  </select>
                  <button onClick={() => setAvailableShifts((prev) => prev.map((d, i) => i === index ? { ...d, active: !d.active } : d))}
                    className="w-24 px-2 py-1 text-xs rounded text-center font-medium transition-all">
                    <span className={`px-2 py-1 rounded-full border transition-all ${day.active ? "bg-blue-100 text-blue-700 border-blue-200" : "bg-gray-100 text-gray-600 border-gray-200"}`}>
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Want to look for jobs instead? Switch your account to a student. This will permanently delete all your employer data.
              </p>
            </div>
            <Button
              variant="outline"
